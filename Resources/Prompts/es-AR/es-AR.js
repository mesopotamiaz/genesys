...
/*
 *****************************************************************************
 * 							es-AR.js (Argentina - Spanish)
 * ---------------------------------------------------------------------------
 * 	This file is built for Argentina Spanish.
 * 
...
function esARPlayBuiltinType(value, type, promptUrl, format)
{
    ...
    return PlayBuiltinType(value, type, format)
}

/**
* Traduce fechas de tipo 2014/12/30 o 16/06/2017
*/
function fechaConSeparadoresAFechaIVR(fecha) {
    if (null !== fecha && fecha.length == 8) {
        return fecha; /* Ej: 20060913*/
    }
    ...
    if (fecha.indexOf("/") == 2) {
    	/* ej: 16/06/2017 */
    	try {
    		ret = fecha.substring(6, 10) + fecha.substring(3, 5) + fecha.substring(0, 2);
    	} catch (e) {
    		ret = '';
    	}
    	return ret;
    ...
    try {
        ret = fecha.substring(0, 4) + fecha.substring(5, 7) + fecha.substring(8, 10);
    } catch (e) {
        ret = '';
    }
    return ret;
...
/**
* Traduce importes en formato moneda Argentina a formato genesys
* Ej: 1.100,05 -> 1100.05
*/
function normalizarCurrency(numStr) {
    var regex = /^(.{3})?(-)?\d{1,3}(\.\d{3})*((,\d*)?)$/;
    var regex2 = /^(.{3})?(-)?\d+(\.\d{3})*,(\d*)$/;
    numStr = '00' + numStr;
    if (regex.test(numStr) || regex2.test(numStr)) {
...
function PlayBuiltinType(value, type, format)
{
    ...
    switch (type) {
        ...
        case 'date':
            return datePrompts(fechaConSeparadoresAFechaIVR(value), format);
        case 'digits':
            return alphanumericPrompts(value);
        case 'currency':
            return currencyPrompts(normalizarCurrency(value));
        case 'number':
        ...
        case 'alphanumeric':
            return alphanumericPrompts2(value);
        case 'dtmf':
...
function datePrompts(value, format)
{
    ...
    if (yearKnown && monthKnown && dayKnown) {
        ...
        if (speakDay) {
            promptsArray = promptsArray.concat(cardinalPrompts(day));
        }
        if (speakMonth) {
            promptsArray.push(promptBaseUrl + "miscellaneous/de.vox");
            promptsArray = promptsArray.concat(monthPrompts(month));
        }
        if (speakYear) {
            promptsArray.push(promptBaseUrl + "miscellaneous/de.vox");
...
function currencyPrompts(value)
{
    ...
    if (amount > 0) {
        promptsArray = promptsArray.concat(cardinalPrompts(amount,true));
    ...
    if (fraction > 0) {
    	if (fraction.length == 1)
        	fraction += '0';
        promptsArray = promptsArray.concat(cardinalPrompts(fraction,true));
...
function cardinalPrompts(number,iscurrency)
{
	var numero_real;
	numero_real = number;
    ...
    while (number > 0) {
        ...
        if (endDigits != 0) {
            promptsArray = threeDigitsPrompts(endDigits,numero_real,iscurrency).concat(magnitudePrompts(magnitude), promptsArray);
        }
...
function threeDigitsPrompts(number,numero_real,iscurrency)
{
	if (iscurrency) {
		return threeDigitsPromptsCurrency(number,numero_real);
	}
    ...
    if (tensAndOnes > 0) {
        if (tensAndOnes < 10) {
			if (tensAndOnes == 1) {
				if (numero_real > 999 && numero_real < 2000 && hundreds == 0)
				{
				} else {			
...
function threeDigitsPromptsCurrency(number,numero_real)
{	
    // assert number >=0 && number <= 999
    if (number < 0 || number > 999) {
        return void 0;
    ...
    if (tensAndOnes > 0) {
        if (tensAndOnes < 10) {
        	...
        	if (ones == 1 && tens > 10) {        		
        			promptsArray.push(promptBaseUrl + "cardinals/0" + tensAndOnes + "n" + audioFileExtension);        			
        	} else {
        		promptsArray.push(promptBaseUrl + "cardinals/0" + tensAndOnes + "" + audioFileExtension);
        	}
        }
...
function alphanumericPrompts2(string)
{
    var i;
    var ch;
	var mistring;
	...
	if (string.substring(0,8) == 'dynamic:'){
		
		
        if (strEndsWith(string,';')) {
            mistring=string.substring(0,string.length - 1); //le resto 1 para sacar el ultimo ";" que viene en el string
        }
		//alert (mistring);
        
        mistring= mistring.substring(8,string.length); /* Quito el dynamic: */

	    var prompts = mistring.split(";");
		var largo = prompts.length;
		//alert (largo);
		
		for (i = 0; i < largo; i++) {
			//alert (prompts[i]);

		    var unprompt = prompts[i].split(","); //busco el dato y el tipo de dato a reproducir
			var dato = unprompt[0];
			var tipo = unprompt[1];
			...
			if (tipo == 'voxfile' || tipo == 'audio') {
				//promptsArray.push(promptBaseUrl + "audios/phrases_Unificacion_OPEN/" + dato);
				promptsArray.push(promptBaseUrl + "audios/phrases_VentasIN/" + dato);
			} else{
				if (tipo == 'date') {
					promptsArray = promptsArray.concat(PlayBuiltinType(dato, tipo, unprompt[2]));
				} else {
					promptsArray = promptsArray.concat(PlayBuiltinType(dato, tipo));
				}
			}
		...
		return alphanumericPrompts(string); //llamo a la rutina estandar de Genesys para el tratamiento de strings
	}    
}
